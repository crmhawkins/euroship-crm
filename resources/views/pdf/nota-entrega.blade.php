...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { padding: 16px 30px 12px; border-bottom: 3px solid #1E3A8A; }
        .header-inner { display: flex; justify-content: space-between; align-items: center; }
        .company-block { text-align: right; font-size: 9px; color: #555; line-height: 1.7; }
        .company-name { font-size: 14px; font-weight: bold; color: #1E3A8A; display: block; margin-bottom: 3px; }

        .title-bar { background: #1E3A8A; color: #fff; padding: 8px 30px; font-size: 15px; font-weight: bold; letter-spacing: 3px; margin-bottom: 16px; }

        .info-row { display: flex; margin: 0 30px 14px; border: 1.5px solid #cbd5e1; }
        .info-cell { flex: 1; padding: 9px 14px; border-right: 1px solid #cbd5e1; }
        ...
        .remarks-block { margin: 0 30px 14px; padding: 9px 14px; border-left: 3px solid #1E3A8A; background: #f0f4ff; }
        ...
        table thead th { background: #1E3A8A; color: #fff; padding: 6px 9px; text-align: left; font-size: 9px; font-weight: bold; text-transform: uppercase; }
        table thead th.center { text-align: center; }
        table thead th.right { text-align: right; }
        table tbody td { padding: 6px 9px; border-bottom: 1px solid #e5e7eb; font-size: 10px; vertical-align: middle; line-height: 1.4; }
        ...
        table tfoot td { padding: 6px 9px; border-top: 2px solid #1E3A8A; font-size: 10px; font-weight: bold; background: #e8eeff; }
        ...
        .cb-item { display: flex; align-items: center; gap: 7px; font-size: 11px; font-weight: bold; text-transform: uppercase; }
        .cb-box { width: 14px; height: 14px; border: 1.5px solid #374151; display: inline-block; flex-shrink: 0; text-align: center; line-height: 12px; font-size: 11px; }
        .cb-checked { background: #1E3A8A; color: #fff; border-color: #1E3A8A; }

        .sign-row { margin: 0 30px 20px; display: flex; justify-content: flex-end; }
        .sign-box { border: 1px solid #9ca3af; padding: 90px 20px 10px; text-align: center; width: 240px; }
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="font-size:9px;">{{ $servicio->number ?? '&mdash;' }}</td>
                <td class="center">{{ $servicio->bx ?? '&mdash;' }}</td>
                <td class="right">{{ $servicio->kg ?? '&mdash;' }}</td>
            </tr>
            @empty
            <tr>
                <td colspan="6" style="text-align:center;color:#aaa;padding:16px;">No shipment records found for this port call.</td>
            </tr>
            @endforelse
        </tbody>
...
@php
    $overtime       = $escala?->overtime ?? false;
    $handlingExpress = $escala?->handling_express ?? false;
    $craneService   = $escala?->crane_service ?? false;
@endphp
...
<div class="checkboxes">
    ...
    <div class="checkbox-row">
        <div class="cb-item">
            <span class="cb-box {{ $overtime ? 'cb-checked' : '' }}">{{ $overtime ? '&#10003;' : '' }}</span>
            Overtime
        </div>
        <div class="cb-item">
            <span class="cb-box {{ $handlingExpress ? 'cb-checked' : '' }}">{{ $handlingExpress ? '&#10003;' : '' }}</span>
            Handling / Express
        </div>
        <div class="cb-item">
            <span class="cb-box {{ $craneService ? 'cb-checked' : '' }}">{{ $craneService ? '&#10003;' : '' }}</span>
            Crane Service
        </div>
    </div>
</div>
